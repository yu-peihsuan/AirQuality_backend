...
```
2026-08-27T12:34:32+08:00
```

修正前是 `2026-08-27T12:34:32.227786`——不帶位移、且有微秒。

兩處都改了，因為 App 的 `MapScreen` 用
`SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssZ")` 解析：格式中的 `Z` 要求必須有
位移，而 `ss` 之後緊接著時區、中間不能有 `.123456`。**兩個條件都不滿足，
所以它對民眾回報的時間戳一直解析失敗。** 補上位移並改成秒精度後才真的能用。
...
25 項檢查，涵蓋基準位移與時間戳格式、三個時間窗的邊界、新舊格式相容、
